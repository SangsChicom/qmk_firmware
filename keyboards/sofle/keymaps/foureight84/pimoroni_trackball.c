...
#   define TRACKBALL_ACCELERATION_WINDOW 200 // ms window to increase acceleration factor
#endif
...
__attribute__((weak)) void process_mouse(report_mouse_t* mouse) {
    ...
    if (trackball_get_interrupt() && (!tb_timer || timer_elapsed(tb_timer) > TRACKBALL_TIMEOUT)) {
        ...
        if (state.button_triggered) {
            ...
            float power = 1.2;
            float magnitude = 5;
            float var_accel = 1.5; //acceleration factor
            double newlen = pow(state.vector_length, power);

            if (state.vector_length > threshold && (timer_elapsed(acceleration_timer) == 0 || timer_elapsed(acceleration_timer) < TRACKBALL_ACCELERATION_WINDOW)) {
                acceleration_timer = timer_read();
                newlen += pow(state.vector_length*var_accel, power)*magnitude;
            } else {
                acceleration_timer = timer_read();
                newlen += pow(state.vector_length, power)*magnitude;
            }
